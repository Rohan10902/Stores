...
Item {
    ...
    FileDialog {
        ...
        onAccepted: {
            if (typeof backend !== "undefined" && backend.health) {
                backend.health.export_health_report(urlToPath(selectedFile))
            }
        }
    }
    ...
    Connections {
        ...
        function onTableReady(payload) {
            try {
                ...
                availableCols = d.columns || []
                statsResult = null // clear previous stats
            } catch(e) {}
        }

        function onHealthReady(payload) {
            try {
                statsResult = JSON.parse(payload)
            } catch(e) {}
        }
    ...
    ColumnLayout {
        ...
        PageTitle {
            ...
            subtitle: "Profile dataset health and run statistical operations."
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    ComboBox {
                        id: groupCombo
                        Layout.preferredWidth: 150
                        model: ["(None)"].concat(availableCols)
                        background: Rectangle { color: Theme.background; border.color: Theme.border; radius: Theme.radiusMedium }
                        contentItem: Text { text: parent.currentIndex >= 0 ? parent.currentText : ""; color: Theme.textPrimary; verticalAlignment: Text.AlignVCenter; leftPadding: 8 }
                    }
                    ...
                    AppButton { 
                        ...
                        onClicked: {
                            if (backend && backend.health) {
                                backend.health.stats(colCombo.currentText, opCombo.currentText, groupCombo.currentText === "(None)" ? "" : groupCombo.currentText)
                            }
                        }
                    }
                }
        ...
        Card {
            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    color: Theme.background; border.color: Theme.border; radius: Theme.radiusMedium
                    ...
                    ScrollView {
                        ...
                        clip: true
                        Text {
                            text: statsResult ? JSON.stringify(statsResult, null, 2) : "Run an operation to view statistics."
                            color: statsResult ? Theme.textPrimary : Theme.textMuted
