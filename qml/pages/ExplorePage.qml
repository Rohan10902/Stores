...
Item {
    ...
    Connections {
        target: backend
        function onHealthReady(p) { var d=JSON.parse(p); cols=d.columnNames; searchCol.model=["All columns"].concat(cols); refreshSuggestions() }
        function onTableReady(p) {
            var d=JSON.parse(p); resultCols=d.columns; tableRows.clear()
    ...
    ColumnLayout {
        ...
        Card {
            Layout.fillWidth:true; implicitHeight:245
            ColumnLayout {
                anchors.fill:parent; anchors.margins:10; spacing:8
                RowLayout {
                    TextField { id:search;placeholderText:"Search records...";Layout.fillWidth:true;Keys.onReturnPressed:backend.search(search.text,searchCol.currentIndex<=0?"":searchCol.currentText) }
                ...
                RowLayout {
                    ...
                    ComboBox { id:suggestion;Layout.fillWidth:true;onActivated:{sql.text=currentText} }
                    AppButton { text:"Use Suggestion";enabled:suggestion.currentText!=="";onClicked:sql.text=suggestion.currentText }
                    AppButton { text:"Clear";onClicked:sql.clear() }
                }
                TextArea {
                    id:sql; text:"SELECT * FROM data LIMIT 100"; color:"#f8fafc"; Layout.fillWidth:true; Layout.fillHeight:true
                    wrapMode:TextEdit.NoWrap; selectByMouse:true; font.family:"Consolas"
                    Keys.onPressed:function(event){if((event.modifiers&Qt.ControlModifier)&&event.key===Qt.Key_Return){backend.sql(sql.text);event.accepted=true}}
                    background:Rectangle{color:"#071321";border.width:1;border.color:"#263850";radius:6}
                }
                RowLayout {
                    Text { text:"Read-only analysis workspace • Ctrl+Enter runs query";color:"#60a5fa";Layout.fillWidth:true }
                    PrimaryButton { text:"Run SQL";enabled:src!==""&&sql.text.trim()!=="";onClicked:backend.sql(sql.text) }
                }
            }
        }
