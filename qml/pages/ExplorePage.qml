...
Item {
    ...
    function quotedColumns() {
        var out = []
        for (var i = 0; i < cols.length; ++i) out.push(qi(cols[i]))
        return out.join(", ")
    }
    ...
    function buildSuggestions(profile) {
        ...
        if (cols.length>0) {
            addSuggestion("Find exact duplicate rows", "Shows duplicated records so they can be reviewed", "SELECT * FROM data WHERE rowid IN (SELECT MIN(rowid) FROM data GROUP BY " + quotedColumns() + " HAVING COUNT(*) > 1) LIMIT 100")
            var missing=[]
    ...
    Connections {
        target: backend
        function onHealthReady(p) { var d=JSON.parse(p); cols=d.columnNames; searchCol.model=["All columns"].concat(cols); buildSuggestions(d) }
    ...
    ColumnLayout {
        ...
        Card {
            ...
            ColumnLayout {
                ...
                RowLayout { visible:suggestions.count>0; Layout.fillWidth:true; Text { text:"Suggested Queries"; color:"#f8fafc"; font.bold:true } Text { text:"Generated from the loaded dataset. Click a suggestion to preview valid SQL."; color:"#94a3b8"; Layout.fillWidth:true } AppButton { text:"Run Selected"; enabled:suggestionList.currentIndex>=0; onClicked:{ if(suggestionList.currentIndex>=0){sql.text=suggestions.get(suggestionList.currentIndex).query;backend.sql(sql.text)} } } }
                ListView {
                    id:suggestionList; visible:suggestions.count>0; Layout.fillWidth:true; Layout.preferredHeight:90; orientation:ListView.Horizontal; spacing:8; clip:true; model:suggestions
                    delegate:Rectangle {
