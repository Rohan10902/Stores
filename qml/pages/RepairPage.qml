...
import "../components"
Item{
 property string src:"";property var audit:({});property int selected:-1;property bool mappingMode:false;property bool newRecordMode:false
 ...
 function showIssue(i){selected=i;mappingMode=false;newRecordMode=false;preview.clear();if(i<0||i>=issues.count)return;var c=JSON.parse(issues.get(i).columnsJson);for(var j=0;j<c.length;j++){var x=c[j];preview.append({sourceIndex:j,field:String(x.field),before:String(x.detected),after:String(x.proposed||""),state:String(x.state),reason:String(x.reason||""),suggested:String(x.suggestedField||""),confidence:String(x.confidence||0),decision:String(x.decision||""),candidatesJson:JSON.stringify(x.candidates||[]),target:String(x.suggestedField||"")})}}
 function createNewRecord(){var m={"__values__":{"SID":newSid.text}};for(var i=0;i<preview.count;i++){var r=preview.get(i);if(r.field.indexOf("PRESERVED EXTRA")===0&&r.target)m[String(r.sourceIndex)]=r.target}backend.createRepairRecord(selected,JSON.stringify(m));newRecordMode=false}
 ...
 ColumnLayout{anchors.fill:parent;anchors.margins:22;spacing:10
  ...
  SplitView{Layout.fillWidth:true;Layout.fillHeight:true;orientation:Qt.Vertical
   ...
   Card{SplitView.minimumHeight:300;SplitView.fillHeight:true;ColumnLayout{anchors.fill:parent;anchors.margins:10
    ...
    RowLayout{visible:selected>=0;Layout.fillWidth:true
     Text{text:"Record decision";color:"#94a3b8";font.bold:true}
     AppButton{text:mappingMode?"Hide Mapping":"Repair Current Record";onClicked:{mappingMode=!mappingMode;newRecordMode=false}}
     AppButton{text:newRecordMode?"Cancel New Record":"Split / Create New Record";onClicked:{newRecordMode=!newRecordMode;mappingMode=newRecordMode}}
     AppButton{text:"Keep Entire Record";onClicked:backend.keepRepairIssue(selected)}
     TextField{id:newSid;visible:newRecordMode;Layout.preferredWidth:150;placeholderText:"New SID (required)"}
     PrimaryButton{visible:newRecordMode;text:"Create New Record Candidate";enabled:newSid.text.trim().length>0;onClicked:createNewRecord()}
     Item{Layout.fillWidth:true}
     Text{text:newRecordMode?"Map only values that belong to the new record. Blank fields are never invented.":mappingMode?"Choose destinations only where needed.":"Select an action to avoid unnecessary controls.";color:"#60a5fa"}
    }
    ...
    ListView{Layout.fillWidth:true;Layout.fillHeight:true;model:preview;clip:true;delegate:Rectangle{
     ...
     RowLayout{anchors.fill:parent;spacing:8
      Text{text:field;color:"#f8fafc";font.bold:true;Layout.preferredWidth:175;Layout.leftMargin:6;elide:Text.ElideRight}
      ...
      RowLayout{Layout.preferredWidth:210;visible:mappingMode&&field.indexOf("PRESERVED EXTRA")===0
       ComboBox{id:dest;Layout.preferredWidth:120;model:audit.header||[];currentIndex:target?(audit.header||[]).indexOf(target):-1;onCurrentTextChanged:{if(currentIndex>=0)preview.setProperty(index,"target",currentText)}}
       Button{visible:!newRecordMode;text:"Apply";enabled:dest.currentIndex>=0&&state!=="USER APPROVED";onClicked:backend.applyRepairMapping(selected,sourceIndex,dest.currentText,true)}
       Button{visible:!newRecordMode;text:"Keep";enabled:state!=="USER APPROVED";onClicked:backend.keepRepairUnresolved(selected,sourceIndex)}
      }
      Text{visible:field.indexOf("PRESERVED EXTRA")!==0||!mappingMode;text:state;color:state==="USER APPROVED"?"#22c55e":"#94a3b8";Layout.preferredWidth:210}
     }
    }}
   }}
  }
