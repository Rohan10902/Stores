...
Item {
 ...
 property int selectedRow:0;property int selectedCol:0;property int validationCount:0;property string undoJson:"";property string notice:""
 ListModel{id:grid} ListModel{id:validation}
 ...
 function cellColor(row,col){if(selectedRow===row&&selectedCol===col)return "#17375f";return row%2?"#0d1b2e":"#0b1829"}
 ...
 ColumnLayout{anchors.fill:parent;anchors.margins:18;spacing:8
  ...
  Text{text:"Fixed 14-column output schema. Rows are virtualized so large store lists do not create every cell on screen at once.";color:"#94a3b8"}
  RowLayout{Layout.fillWidth:true;PrimaryButton{text:"Paste from Clipboard";onClicked:pasteText(backend.clipboardText())}AppButton{text:"Add Row";onClicked:{saveUndo();addBlank()}}AppButton{text:"Delete Selected Row";enabled:grid.count>1;onClicked:{saveUndo();grid.remove(selectedRow);selectedRow=Math.max(0,Math.min(selectedRow,grid.count-1));backend.validateCreator(rowsJson())}}AppButton{text:"Clear Table";onClicked:clearAll()}AppButton{text:"Undo";enabled:undoJson!=="";onClicked:restoreUndo()}Item{Layout.fillWidth:true}Text{text:grid.count+" rows";color:"#94a3b8"}AppButton{text:"Validate";onClicked:backend.validateCreator(rowsJson())}PrimaryButton{text:"Export CSV";enabled:grid.count>0;onClicked:saveDlg.open()}}
  Card{Layout.fillWidth:true;implicitHeight:92;ColumnLayout{anchors.fill:parent;anchors.margins:8
   RowLayout{Layout.fillWidth:true;Text{text:"Bulk leading zeros";color:"#f8fafc";font.bold:true}ComboBox{id:bulkCol;model:headers;Layout.preferredWidth:220;currentIndex:3}SpinBox{id:padWidth;from:1;to:20;value:11;editable:true}PrimaryButton{text:"Apply Padding";onClicked:padColumn()}Item{Layout.fillWidth:true}Text{text:"Works for SID, Nielsen Store Code, ZIP, or any selected all-digit column";color:"#60a5fa"}}
   RowLayout{Layout.fillWidth:true;Text{text:"Find / Replace";color:"#f8fafc";font.bold:true}TextField{id:findText;placeholderText:"Find";Layout.preferredWidth:180}TextField{id:replaceText;placeholderText:"Replace with";Layout.preferredWidth:180}ComboBox{id:replaceCol;model:["All columns"].concat(headers);Layout.preferredWidth:220}PrimaryButton{text:"Replace All";onClicked:replaceAll()}Item{Layout.fillWidth:true}}
  }}
  ...
  Card{Layout.fillWidth:true;Layout.fillHeight:true;clip:true
   Flickable{id:hscroll;anchors.fill:parent;anchors.margins:8;contentWidth:headers.length*155;contentHeight:height;clip:true;flickableDirection:Flickable.HorizontalFlick
    ...
    ListView{id:rowList;y:headerRow.height;width:headers.length*155;height:hscroll.height-headerRow.height;model:grid;clip:true;cacheBuffer:300;reuseItems:true
     delegate:Row{required property int index;property int rr:index;width:headers.length*155;height:38
      Repeater{model:headers.length;delegate:TextField{required property int index;property int cc:index;width:155;height:38;padding:6;text:grid.get(parent.rr)["c"+cc]||"";selectByMouse:true;background:Rectangle{color:page.cellColor(parent.parent.rr,cc);border.color:"#29415f"}color:"#f8fafc";onActiveFocusChanged:{if(activeFocus){selectedRow=parent.parent.rr;selectedCol=cc}}onEditingFinished:{saveUndo();grid.setProperty(parent.parent.rr,"c"+cc,text)}}}
     }
