...
Item {
    ...
    function resetRows() {
        var nextRows = []
        var nextSelected = []
        for (var i = 0; i < root.minimumRows; ++i) {
            nextRows.push(root.blankRow())
            nextSelected.push(true)
        }
        root.rows = nextRows
        root.selected = nextSelected
        root.findings = []
    ...
    function addRow() {
        var next = root.rows.slice()
        var chosen = root.selected.slice()
        next.push(root.blankRow())
        chosen.push(true)
        root.rows = next
    ...
    function deleteSelected() {
        ...
        for (var i = 0; i < root.rows.length; ++i) {
            if (!root.selected[i]) {
                next.push(root.rows[i])
                chosen.push(false)
            }
        }
        while (next.length < root.minimumRows) {
            next.push(root.blankRow())
            chosen.push(true)
        }
        root.rows = next
    ...
    function enteredCount() {
        var count = 0
        for (var i = 0; i < root.rows.length; ++i)
            if (root.hasData(root.rows[i])) ++count
        return count
    }
    function includedCount() {
        var count = 0
        for (var i = 0; i < root.rows.length; ++i)
            if (root.selected[i] && root.hasData(root.rows[i])) ++count
        return count
    }
    ...
    function loadImported() {
        ...
        for (var r = 0; r < root.importedRows.length; ++r) {
            var source = root.importedRows[r] || []
            var target = root.blankRow()
            for (var c = 0; c < root.importedHeaders.length; ++c) {
                var targetIndex = root.mapIndex(root.importedHeaders[c])
                if (targetIndex >= 0)
                    target[targetIndex] = source[c] === undefined || source[c] === null ? "" : String(source[c]).trim()
            }
            if (root.hasData(target)) {
                nextRows.push(target)
                nextSelected.push(true)
            }
        }
        while (nextRows.length < root.minimumRows) {
            nextRows.push(root.blankRow())
            nextSelected.push(true)
        }
        root.rows = nextRows
        root.selected = nextSelected
        root.invalidate()
        root.importPreviewVisible = false
    }
    function validationJson() {
        ...
        for (var r = 0; r < root.rows.length; ++r) {
            ...
            var record = {}
            for (var c = 0; c < root.headers.length; ++c)
                record[root.headers[c]] = String(root.rows[r][c] || "").trim()
            output.push(record)
        }
        return JSON.stringify(output)
    }
    function validateRows() {
        if (!root.includedCount() || root.validationPending || !root.backendAvailable()) return
        root.validationPending = true
        root.validated = false
        root.findings = []
        backend.creator.validate_creator(root.validationJson())
    }
    function exportRows() {
        if (!root.validated || root.findings.length > 0 || !root.includedCount() || root.exporting || !root.backendAvailable()) return
        var output = []
        for (var i = 0; i < root.rows.length; ++i)
            if (root.selected[i] && root.hasData(root.rows[i])) output.push(root.rows[i])
        root.exporting = true
    ...
    Popup {
        ...
        ColumnLayout {
            ...
            RowLayout {
                Layout.fillWidth: true
                Text { text: "Excel clipboard data is normally tab-separated."; color: Theme.textMuted; Layout.fillWidth: true }
    ...
    Connections {
        ...
        function onCreatorLoaded(payload) {
            ...
            try {
                ...
                root.importedTotal = Number(data.total || root.importedRows.length || 0)
        ...
        ColumnLayout {
            ...
            Card {
                ...
                RowLayout {
                    ...
                    ColumnLayout {
                        spacing: 1
                        Text { text: root.enteredCount() + " entered"; color: Theme.textPrimary; font.bold: true; Layout.alignment: Qt.AlignRight }
                        Text { text: root.includedCount() + " included • " + root.headers.length + " columns"; color: Theme.textSecondary; font.pixelSize: 10; Layout.alignment: Qt.AlignRight }
                    }
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        ColumnLayout {
                            ...
                            Text { text: root.importPending ? "Reading the selected file." : root.importedTotal + " records • " + root.importedHeaders.length + " source columns • " + root.mappedCount() + " mapped"; color: Theme.textSecondary; font.pixelSize: 11 }
                        }
                        AppButton { text: "Hide"; onClicked: root.importPreviewVisible = false }
                        PrimaryButton { text: "Load into Store Builder"; enabled: root.importedHeaders.length > 0 && !root.importPending && root.mappedCount() > 0; onClicked: root.loadImported() }
                    }
                    Rectangle {
