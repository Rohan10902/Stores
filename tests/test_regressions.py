...
class RegressionTests(unittest.TestCase):
    def test_multiline_csv_and_duplicate_headers(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d)/"x.csv";p.write_text('SID,Notes,Notes\n001,"hello\nworld",x\n',encoding='utf-8');df=read_table(p);self.assertEqual(len(df),1);self.assertEqual(list(df.columns),['SID','Notes','Notes (2)']);self.assertEqual(df.iloc[0]['Notes'],'hello\nworld')
    ...
    def test_builder_accepts_active_inactive_labels(self):
        row={'Store Name':'A','SID':'001','Active / Inactive':'Active','Is Census':'Yes','Is Exceptions':'No','Trip Received':'2026-07-01','Last Trip':'2026-07-02'}
    ...
    def test_builder_handles_100_valid_records_without_findings(self):
        ...
        for i in range(100):
            n=i+1
            rows.append({'Store Name':f'Store {n:03d}','SID':f'{100000+n}','Banner':'Test Banner','Nielsen Store Code':f'{100000000+n}','Trip Received':'2026-07-01','Last Trip':'2026-07-02','Address 1':f'{n} Main Street','ZIP':f'{10000+n:05d}','Active / Inactive':'Active','Is Census':'Yes' if n % 2 else 'No','Is Exceptions':'No','Updated By':'Test'})
