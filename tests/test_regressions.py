...
from core.controllers.review_controller import _read_tolerant_csv
...
def test_review_tolerates_extra_csv_fields_and_reports_finding(tmp_path):
    csv_file = tmp_path / "review_extra.csv"
    csv_file.write_text("SID,Store Name\n101,Branch A,Unexpected\n102,Branch B", encoding="utf-8")
    dataframe, findings = _read_tolerant_csv(str(csv_file))
    assert list(dataframe.columns) == ["SID", "Store Name"]
    assert dataframe.iloc[0].tolist() == ["101", "Branch A"]
    assert len(findings) == 1
    assert findings[0]["severity"] == "ERROR"
    assert "expected 2 fields, found 3" in findings[0]["message"]


def test_review_tolerates_missing_csv_fields_and_reports_finding(tmp_path):
    csv_file = tmp_path / "review_missing.csv"
    csv_file.write_text("SID,Store Name\n101\n102,Branch B", encoding="utf-8")
    dataframe, findings = _read_tolerant_csv(str(csv_file))
    assert dataframe.iloc[0].tolist() == ["101", ""]
    assert len(findings) == 1
    assert "expected 2 fields, found 1" in findings[0]["message"]
