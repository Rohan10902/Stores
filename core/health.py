...
def infer_type(s):
    ...
    # Require date-like separators/text to avoid numeric IDs becoming dates.
    txt=x.astype(str)
...
def _insight(df,col,typ,op,rows):
    s=df[col]; total=len(s); blanks=int(_blank(s).sum()); clean=s[~_blank(s)]
    bits=[]
    ...
    if typ=="numeric" and len(clean):
        ...
        vc=clean.astype(str).value_counts()
        top=vc.iloc[0]; names=", ".join(map(str,vc[vc==top].index[:4]))
...
def statistic(df,col,op,group=""):
    ...
    if op=="Frequency Distribution":
        ...
        for k,g in df.groupby(group,dropna=False):
            rows.append({"label":"(blank)" if pd.isna(k) else str(k),"result":_scalar(g[col],op,typ),
                         "count":len(g),"percent":round(len(g)/max(1,len(df))*100,1),"interpretation":op})
    else:
        val=_scalar(df[col],op,typ); count="";percent=""
        clean=df[col][~_blank(df[col])]
        if op in ("Most Common Value","Least Common Value") and len(clean):
            count=int((clean.astype(str)==str(val)).sum());percent=round(count/len(clean)*100,1)
