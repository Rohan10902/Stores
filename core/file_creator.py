...
def empty_rows(n=10): return [{f:"" for f in STORE_FIELDS} for _ in range(max(1,n))]
...
def _canonical_field(value):
    ...
    for field in STORE_FIELDS:
        if n==norm_name(field) or any(n==norm_name(a) for a in ALIASES.get(field,[])):return field
    return ""

def _detect_structure(df):
    columns=[str(c) for c in df.columns];horizontal=sum(1 for c in columns if _canonical_field(c));vertical=[]
    for c in df.columns:
        matches=[]
    ...
    if vertical:
        key_col,matches=max(vertical,key=lambda x:len(x[1]));fields=[]
        for _,f in matches:
...
def _suggested_width(codes):
    widths=[len(x) for x in codes if x.isdigit()]
    if not widths:return 0
    counts=Counter(widths);top=max(counts.values());candidates=[w for w,n in counts.items() if n==top]
    return max(candidates)

def review_dataframe(df):
    rows=[];structure=_detect_structure(df)
    ...
    codes=[norm_value(x) for x in df["Nielsen Store Code"] if norm_value(x)] if "Nielsen Store Code" in df.columns else []
    suggested=_suggested_width(codes)
    for ix,r in df.iterrows():
        item={"row":int(ix)+2,"severity":"OK","issues":[]}
        sid=norm_value(r.get("SID","")) if "SID" in df.columns else ""
        name=norm_value(r.get("Store Name","")) if "Store Name" in df.columns else ""
        if "SID" in df.columns and not sid:item["issues"].append("SID: required value is blank")
        if "Store Name" in df.columns and not name:item["issues"].append("Store Name: required value is blank")
        ...
        if "Nielsen Store Code" in df.columns:
            ...
            if suggested and code.isdigit() and len(code)!=suggested:item["issues"].append(f"Nielsen Store Code: {code} has width {len(code)}; dominant width is {suggested}")
        if item["issues"]:item["severity"]="REVIEW"
        rows.append(item)
    return {"rows":rows,"issueCount":sum(bool(x["issues"]) for x in rows),"findingCount":sum(len(x["issues"]) for x in rows),"suggestedNielsenWidth":suggested,"columns":[str(c) for c in df.columns],"structure":structure,"recordCount":int(len(df))}
...
def creator_validate(rows):
    ...
    for i,row in enumerate(rows):
        ...
        for f in ("Active / Inactive","Is Census","Is Exceptions"):
            v=norm_value(row.get(f,""))
            if not binary_ok(v):findings.append({"row":i+1,"field":f,"value":v,"message":"Expected boolean: true or false"})
