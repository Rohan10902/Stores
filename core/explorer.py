...
def _typed(s,name=""):
    ...
    if low and low.issubset(_TRUE|_FALSE):
        x=s.astype(str).str.strip().str.lower().map(lambda v: True if v in _TRUE else False if v in _FALSE else None);return x.astype("boolean"),"BOOLEAN"
    n=pd.to_numeric(text,errors="coerce")
    if n.notna().all():
        full=pd.to_numeric(s.where(~blank,None),errors="coerce");z=full.dropna()
