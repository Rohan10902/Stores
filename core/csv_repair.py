    ...
    for no,line in enumerate(lines[1:],2):
        ...
        if start!=no and len(vals)==expected:
            for i,v in enumerate(vals):
                cols.append({"field":header[i],"detected":v,"proposed":v,"state":"RECONSTRUCTED","reason":"Joined quoted physical lines","suggestedField":"","confidence":100,"decision":"AUTO FIXED"})
            ...
            for i,v in enumerate(vals):
                extra=i>=expected
                cols.append({"field":header[i] if not extra else f"PRESERVED EXTRA {i-expected+1}","detected":v,"proposed":v if not extra else "","state":"REVIEW" if extra else "UNCHANGED","reason":"No destination selected" if extra else "Position matches header","suggestedField":"","confidence":0,"decision":"REVIEW" if extra else "UNCHANGED"})
            ...
            for i in range(expected):
                present=i<len(vals)
                cols.append({"field":header[i],"detected":vals[i] if present else "","proposed":vals[i] if present else "","state":"UNCHANGED" if present else "MISSING","reason":"Position matches header" if present else "Expected field absent; no value invented","suggestedField":"","confidence":0,"decision":"REVIEW" if not present else "UNCHANGED"})
    ...
    for issue in issues:
        if issue["kind"]=="EXTRA_FIELDS":
            ...
            for j,sug in enumerate(suggestions):
                idx=expected+j
                issue["columns"][idx].update({"suggestedField":sug["suggestedField"],"confidence":sug["confidence"],"decision":sug["decision"],"reason":sug["reason"],"candidates":sug.get("candidates",[])})
...
def create_record_from_extras(audit, issue_index, mapping):
    ...
    record_id=max([r.get("id",0) for r in audit.get("createdRecords",[])]+[0])+1
    rec={"start":0,"end":0,"values":new,"raw":"","createdByUser":True,"createdRecordId":record_id,"sourceIssue":issue_index}
    audit["logical"].append(rec)
    ...
    created={"id":record_id,"logicalIndex":len(audit["logical"])-1,"sourceIssue":issue_index,"values":dict(zip(audit["header"],new)),"active":True}
    audit.setdefault("createdRecords",[]).append(created);audit.setdefault("undoStack",[]).append({"action":"CREATE","recordId":record_id})
    issue["decision"]=f"New record #{record_id} created";issue["status"]="REVIEWED"
    return audit
...
def delete_created_record(audit, record_id):
    ...
    audit.setdefault("undoStack",[]).append({"action":"DELETE","recordId":record_id,"record":removed,"logicalIndex":li})
    return audit

def undo_last_created_action(audit):
    stack=audit.setdefault("undoStack",[])
    if not stack:raise ValueError("Nothing to undo.")
    action=stack.pop();rid=action["recordId"]
    target=next((r for r in audit.get("createdRecords",[]) if r["id"]==rid),None)
