...
def inspect_csv(p):
    path=Path(p)
    # Read chunk to sniff delimiter
    with open(path, "r", encoding="utf-8-sig", errors="replace") as f:
        chunk = f.read(16000)
    
    delim=_delim(chunk,path.suffix)
    expected=0
    logical=[]
    issues=[]
    buf=""
    ...
    # FIX: Iterate line by line instead of loading everything into memory
    with open(path, "r", encoding="utf-8-sig", errors="replace") as f:
        first_line = f.readline()
        if not first_line:raise ValueError("File is empty.")
        header=next(csv.reader([first_line],delimiter=delim))
        expected=len(header)
        logical.append({"start":1,"end":1,"values":header,"raw":first_line.rstrip()})
