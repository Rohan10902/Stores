    ...
    def validate(self, keys_json):
        ...
        def success(results):
            ...
            for index, row in enumerate(raw_rows):
                formatted_rows.append({
                    "row": int(row.get("row", index + 1)),
                    "key": str(row.get("key", "")),
                    "status": str(row.get("status", "UNKNOWN")).upper(),
                    "matchType": str(row.get("matchType", "")),
        ...
        def error(exc):
            if self.fail:
                self.fail("Validation Error", str(exc))
            self.validationReady.emit(json.dumps({
                "total": 0, "correct": 0, "review": 0, "errors": 1, "attention": 1,
                "rows": [], "insights": [], "keys": keys,
            }))
        ...
        try:
            ...
            for item in comparisons:
                formatted.append({
                    ...
                    "uploaded": str(item.get("uploaded", "")),
            ...
            self.detailReady.emit(json.dumps({
                ...
                "diffs": sum(1 for item in formatted if item["severity"] != "OK"),
