...
from core.csv_repair import inspect_csv, apply_mapping, keep_unresolved, save_repaired, unresolved_extras
...
class Backend(QObject):
    ...
    detailReady=Signal(str);repairReady=Signal(str);healthReady=Signal(str);statsReady=Signal(str);tableReady=Signal(str)
    def __init__(self):
    ...
    def validate(self,key_json=""):
        try:
            ...
            self.validationReady.emit(json.dumps({"total":len(rows),"correct":sum(r["status"]=="CORRECT" for r in rows),
                ...
                "rows":rows,"keyFields":self.key_fields}))
