...
from core.csv_repair import inspect_csv, apply_mapping, keep_unresolved, save_repaired, unresolved_extras, keep_issue_as_is, create_record_from_extras, preview_rows
...
class Backend(QObject):
    ...
    def _emit_repair(self):
        data={k:v for k,v in self.repair_audit.items() if k!="logical"};data["previewRows"]=preview_rows(self.repair_audit);self.repairReady.emit(json.dumps(data,default=str))
    @Slot(str)
    ...
    def createRepairRecord(self,issue_idx,mapping_json):
        try:create_record_from_extras(self.repair_audit,issue_idx,json.loads(mapping_json or "{}"));self._emit_repair();self.say("New record created and added to export preview")
        except Exception as e:self.fail(e)
    @Slot(str,str)
    def repair(self,src,dst):
        try:
            ...
            save_repaired(self.repair_audit,dest);self.say("Reviewed copy saved; original unchanged")
        except Exception as e:self.fail(e)
    @Slot(str)
    def reviewSingleFile(self,path):
        try:
            local=self._local(path);self.single_review=read_table(local);self.single_review_path=local;report=review_dataframe(self.single_review);report["total"]=int(len(self.single_review))
            if Path(local).suffix.lower() in (".csv",".txt",".tsv"):
                audit=inspect_csv(local);struct=[]
                for x in audit.get("issues",[]):struct.append({"row":x.get("line","?"),"severity":x.get("status","REVIEW"),"problem":x.get("problem","")})
                report["structuralFindings"]=struct;report["issueCount"]=int(report.get("issueCount",0))+len(struct);report["total"]=max(report["total"],audit.get("records",0))
            self.singleReviewReady.emit(json.dumps(report,default=str));self.say(f"Single-file review complete: {report['issueCount']} finding(s) need attention")
        except Exception as e:self.fail(e)
    @Slot(int)
    def normalizeSingleNielsen(self,width):
        try:
            ...
            self.single_review[column]=self.single_review[column].map(lambda v:normalize_nielsen(v,width));report=review_dataframe(self.single_review);report["total"]=int(len(self.single_review));self.singleReviewReady.emit(json.dumps(report,default=str));self.say(f"Preview updated — numeric Nielsen codes padded to width {width}")
        except Exception as e:self.fail(e)
    @Slot(str,str)
...
def main():
    ...
    qml_errors=[]
    def capture_warnings(warnings):
        for warning in warnings:
            text=warning.toString();qml_errors.append(text);logging.error("QML: %s",text);print("QML:",text,file=sys.stderr,flush=True)
