...
class Backend(QObject):
    ...
    def validate(self, key_json=""):
        try:
            ...
            self.validationReady.emit(json.dumps({
                ...
                "rows": rows,
                "keyFields": self.key_fields,
                "insights": intel["groups"],
    ...
    def detail(self, index, differences_only):
        try:
            ...
            self.detailReady.emit(json.dumps({
                "missingMaster": "not found in Master" in rec["problem"],
                "sid": rec["sid"], "rows": rows, "problem": rec["problem"],
                "status": rec["status"], "context": rec["context"]
            }))
        except Exception as e:
            self.fail(e)
    ...
    def inspectRepair(self, path):
        try:
            self.repair_audit = inspect_csv(self._local(path))
            self._emit_repair()
    ...
    def applyRepairMapping(self, issue_idx, col_idx, target, remember):
        try:
            ...
            self._emit_repair()
            self.say(f"Mapped '{value}' → {target} in reviewed copy")
        except Exception as e:
            self.fail(e)
    ...
    def keepRepairUnresolved(self, issue_idx, col_idx):
        try:
            ...
            self._emit_repair()
            self.say("Value kept unresolved")
        except Exception as e:
            self.fail(e)
    ...
    def keepRepairIssue(self, issue_idx):
        try:
            ...
            self._emit_repair()
            self.say("Whole record kept for explicit review")
        except Exception as e:
            self.fail(e)
    ...
    def createRepairRecord(self, issue_idx, mapping_json):
        try:
            ...
            self._emit_repair()
            self.say("New record candidate created from user-approved mappings")
        except Exception as e:
            self.fail(e)
    ...
    def repair(self, src, dst):
        try:
            ...
            if pending:
                raise ValueError(f"{len(pending)} preserved extra value(s) are still unresolved. Map them before saving.")
            dest = self._local(dst)
            if not dest.lower().endswith(".csv"):
                dest += ".csv"
            save_repaired(self.repair_audit, dest)
            self.say("Reviewed copy saved; original unchanged")
        except Exception as e:
            self.fail(e)
    ...
    def reviewSingleFile(self, path):
        try:
            ...
            report["total"] = int(len(self.single_review))
            self.singleReviewReady.emit(json.dumps(report, default=str))
            self.say(f"Single-file review complete: {report['issueCount']} row(s) need attention")
        except Exception as e:
            self.fail(e)
    ...
    def normalizeSingleNielsen(self, width):
        try:
            if self.single_review is None:
                raise ValueError("Choose and analyze a file first.")
            mapping = map_columns(self.single_review.columns)
            column = mapping.get("Nielsen Store Code", {}).get("column", "")
            if not column:
                raise ValueError("Nielsen Store Code column could not be detected.")
            self.single_review[column] = self.single_review[column].map(lambda v: normalize_nielsen(v, width))
            report = review_dataframe(self.single_review)
            report["total"] = int(len(self.single_review))
            self.singleReviewReady.emit(json.dumps(report, default=str))
            self.say(f"Preview updated — numeric Nielsen codes padded to width {width}")
        except Exception as e:
            self.fail(e)
    ...
    def exportSingleReview(self, src, dst):
        try:
            if self.single_review is None or self.single_review_path != self._local(src):
                self.single_review = read_table(self._local(src))
                self.single_review_path = self._local(src)
            dest = Path(self._local(dst))
            if dest.suffix.lower() != ".csv":
                dest = dest.with_suffix(".csv")
            self.single_review.to_csv(dest, index=False, encoding="utf-8-sig")
            self.say(f"Reviewed copy exported: {dest.name}")
        except Exception as e:
            self.fail(e)
    ...
    def validateCreator(self, rows_json):
        try:
            ...
            self.say("Creator validation complete" if not findings else f"Creator validation found {len(findings)} value(s) to review")
        except Exception as e:
            self.fail(e)
    ...
    def loadData(self, path):
        try:
            ...
            self.result = None
            self.healthReady.emit(json.dumps(profile(self.data), default=str))
    ...
    def emit_table(self, data):
        ...
        self.tableReady.emit(json.dumps({"columns": [str(c) for c in data.columns], "rows": rows,
                                         "total": int(len(data)), "displayed": len(rows)}, default=str))
        ...
        try:
            ...
            if term:
                if col and col in data.columns:
                    ...
                    mask = data.fillna("").astype(str).apply(
                        lambda r: r.str.contains(term, case=False, regex=False).any(), axis=1)
                data = data[mask]
            self.emit_table(data)
            self.say(f"Search returned {len(data):,} rows")
        except Exception as e:
            self.fail(e)
    ...
    def sql(self, query):
        try:
            if self.data is None:
                raise ValueError("Load a dataset first.")
            data = run_sql(self.data, query)
            self.emit_table(data)
            self.say(f"Query returned {len(data):,} rows")
        except Exception as e:
            self.fail(e)
