...
class Backend(QObject):
    ...
    qualityReady = Signal(str)

    def __init__(self):
        super().__init__()
        self._message = "Ready"
        self.master = self.upload = self.data = self.result = None
        self.single_review = None
        self.single_review_path = ""
        self.records = []
        self.repair_audit = None
        self.repair_source = ""
    ...
    def loadMaster(self, path):
        try:
            self.master = read_table(self._local(path))
    ...
    def loadUpload(self, path):
        try:
            self.upload = read_table(self._local(path))
            self._reset_validation()
    ...
    def detect(self):
        try:
            if self.master is None or self.upload is None:
                raise ValueError("Load both files first.")
            suggested = suggest_keys(self.master, self.upload)
            self.mappingReady.emit(json.dumps({"master": map_columns(self.master.columns), "upload": map_columns(self.upload.columns), "suggestedKeys": suggested}))
    ...
    def validate(self, key_json=""):
        try:
            ...
            self.validationReady.emit(json.dumps({"total": len(rows), "correct": sum(r["status"] == "CORRECT" for r in rows), "review": sum(r["status"] == "REVIEW" for r in rows), "errors": sum(r["status"] == "ERROR" for r in rows), "rows": rows, "keyFields": self.key_fields, "insights": intel["groups"], "attention": intel["attention"]}))
    ...
    def detail(self, index, differences_only):
        try:
            ...
            rows = rec["comparisons"]
            if differences_only:
                rows = [r for r in rows if r["severity"] != "OK"]
    ...
    def _emit_repair(self):
        ...
        data["canUndo"] = bool(self.repair_audit.get("undoStack"))
        data["records"] = max(0, len(self.repair_audit["logical"]) - 1)
        self.repairReady.emit(json.dumps(data, default=str))

    @Slot(str)
    def inspectRepair(self, path):
        try:
            self.repair_source = self._local(path)
            self.repair_audit = inspect_csv(self.repair_source)
            self._emit_repair()
            self.say("Inspection complete")
        except Exception as e:
            self.fail(e)
    ...
    def joinRepairRows(self, issue_idx):
        try:
            join_shifted_rows(self.repair_audit, issue_idx)
            self._emit_repair()
            self.say("Broken record joined successfully. Undo is available.")
        except Exception as e:
            self.fail(e)
    ...
    def applyRepairMapping(self, issue_idx, col_idx, target, remember):
        try:
            ...
            if remember:
                self.mapping_store.remember(value, target)
            self._emit_repair()
        except Exception as e:
            self.fail(e)
    ...
    def keepRepairUnresolved(self, issue_idx, col_idx):
        try:
            keep_unresolved(self.repair_audit, issue_idx, col_idx)
            self._emit_repair()
        except Exception as e:
            self.fail(e)
    ...
    def keepRepairIssue(self, issue_idx):
        try:
            keep_issue_as_is(self.repair_audit, issue_idx)
            self._emit_repair()
        except Exception as e:
            self.fail(e)
    ...
    def createRepairRecord(self, issue_idx, mapping_json):
        try:
            create_record_from_extras(self.repair_audit, issue_idx, json.loads(mapping_json or "{}"))
            self._emit_repair()
            self.say("New line created successfully. Review it below; Delete and Undo are available.")
        except Exception as e:
            self.fail(e)
    ...
    def deleteRepairRecord(self, record_id):
        try:
            delete_created_record(self.repair_audit, record_id)
            self._emit_repair()
            self.say(f"Created line #{record_id} deleted. Undo is available.")
        except Exception as e:
            self.fail(e)
    ...
    def undoRepairAction(self):
        try:
            undo_last_created_action(self.repair_audit)
            self._emit_repair()
            self.say("Last repair action undone.")
        except Exception as e:
            self.fail(e)
    ...
    def repair(self, src, dst):
        try:
            if self.repair_audit is None:
                raise ValueError("Inspect a file first.")
            if os.path.normcase(os.path.abspath(self._local(src))) != os.path.normcase(os.path.abspath(self.repair_source)):
                raise ValueError("The selected source changed. Inspect it again before saving.")
            pending = unresolved_extras(self.repair_audit)
            if pending:
                raise ValueError(f"{len(pending)} unresolved value(s) remain")
            dest = self._local(dst)
            dest = dest if dest.lower().endswith(".csv") else dest + ".csv"
            save_repaired(self.repair_audit, dest)
            self.say("Reviewed copy saved")
        except Exception as e:
            self.fail(e)

    def _single_report(self):
        report = review_dataframe(self.single_review)
        report["total"] = int(report.get("recordCount", len(self.single_review)))
    ...
    def reviewSingleFile(self, path):
        try:
            ...
            self.single_review = read_table(local)
            self.single_review_path = local
            self.singleReviewReady.emit(json.dumps(self._single_report(), default=str))
            self.say("Single-file analysis complete")
        except Exception as e:
            self.fail(e)
    ...
    def exportSingleReview(self, src, dst):
        try:
            ...
            if self.single_review is None or self.single_review_path != local:
                self.single_review = read_table(local)
                self.single_review_path = local
            output = self.single_review.copy()
            dest = Path(self._local(dst))
            dest = dest if dest.suffix.lower() == ".csv" else dest.with_suffix(".csv")
            output.to_csv(dest, index=False, encoding="utf-8-sig")
    ...
    def clipboardText(self):
        ...
        return app.clipboard().text() if app else ""

    @Slot(str)
    def validateCreator(self, rows_json):
        try:
            findings = creator_validate(json.loads(rows_json or "[]"))
            self.creatorReady.emit(json.dumps({"count": len(findings), "findings": findings}, default=str))
        except Exception as e:
            self.fail(e)
    ...
    def exportCreator(self, rows_json, dst):
        try:
            ...
            if findings:
                raise ValueError(f"Fix {len(findings)} invalid value(s) before export.")
            path = export_creator(rows, self._local(dst))
            self.say("Store CSV exported")
            self.creatorExported.emit(str(path))
        except Exception as e:
            self.fail(e)
    ...
    def loadData(self, path):
        try:
            self.data = read_table(self._local(path))
            health = profile(self.data)
            quality = profile_dataframe(self.data)
            combined = dict(health)
            combined["quality"] = quality
            self.healthReady.emit(json.dumps(combined, default=str))
            self.qualityReady.emit(json.dumps(quality, default=str))
            self.emit_table(self.data)
            self.say(f"Dataset loaded: {len(self.data):,} rows")
        except Exception as e:
            self.fail(e)
    ...
    def emit_table(self, data):
        self.result = data
        rows = [[json_value(x) for x in r] for r in data.head(1000).itertuples(index=False, name=None)]
        self.tableReady.emit(json.dumps({"columns": [str(c) for c in data.columns], "rows": rows, "total": int(len(data)), "displayed": len(rows), "truncated": len(data) > len(rows)}, default=str))
        ...
        try:
            ...
            if term:
                if col and col in data.columns:
                    mask = data[col].fillna("").astype(str).str.contains(term, case=False, regex=False)
                else:
                    mask = data.fillna("").astype(str).apply(lambda r: r.str.contains(term, case=False, regex=False).any(), axis=1)
                data = data[mask]
...
def main():
    ...
    def capture(items):
        for item in items:
            ...
            print("QML:", text, file=sys.stderr, flush=True)
